...
function trainGarage() {
  fetch('http://localhost:5001/train-garage', {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
  });
  alert('Modèle de prédiction pour les garages entraîné avec succès !');
}
...
const response = ref()
async function predictGarage() {
  try {
    const res = await fetch('http://localhost:5001/predict-garage', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    if (res.ok) {
      ...
      alert('Modèle non entraîné pour les garages !')
    }
  } catch (error) {
        ...
        >
          <DialogPanel class="w-full max-w-md transform bg-white p-6 rounded-lg shadow-xl">
            ...
            <div class="mt-4">
              ...
              <a @click="trainGarage()" class="group flex mt-5 items-center px-3 py-2 cursor-pointer text-sm font-medium text-white transition-all duration-200 bg-indigo-600 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 shadow-md hover:shadow-lg">
                ...
                <span class="mr-2">Entrainer le modèle de prédiction pour les garage</span>
              </a>

              <!-- Formulaire pour envoyer des données au backend -->
              <form @submit.prevent="predictGarage">
                <div class="mt-4">
              ...
              <div v-if="response">
                <p class="mt-4 text-sm text-gray-500">
                  Prédiction Garage :
                  {{ response ? (response.has_garage === true ? "Vrai" : "Faux") : 'Aucune prédiction' }}
                </p>
              </div>
            </div>
          </DialogPanel>
